...
def paint():
    ...
    def toggle_color(event):
        ...
        paintwindows.config(bg=colors[current_color_index])

    canvas.bind("<Button-3>", toggle_color)
    ...
    clear_btn = tk.Button(paintwindows, text="Clear", command=clear_canvas)
    clear_btn.place(relx=0.9, rely=0, anchor="ne")

    cancel = tk.Button(paintwindows, text="x", command=paintwindows.destroy, fg="red", height=1, font=("Arial", 10))
    cancel.place(relx=1, rely=0, anchor="ne")
